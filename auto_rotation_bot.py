                ...
                # HOLDING -> Monitor position
                    ...
                    if now >= target_close.replace(tzinfo=None):
                        ...
                        if stop_loss_enabled:
                            logger.info(f"🛡  Stop Loss:     {Colors.YELLOW}{stop_loss_pct:.1f}%{Colors.RESET} (enabled)")
                        else:
                        ...
                        try:
                            # Fetch current funding rates for all monitored symbols with timeout
                            funding_results = await asyncio.wait_for(
                                asyncio.gather(*[
                            ...
                            logger.info(f"{Colors.YELLOW}Funding rate fetch timed out (>30s){Colors.RESET}")
                            logger.info(f"{Colors.GRAY}Skipping funding rates display for this cycle{Colors.RESET}")
                        except Exception as e:
